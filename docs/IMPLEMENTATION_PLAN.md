...
| `requirements/legacy-sources.txt`, `legacy-runtime.txt`, `legacy-build.txt`, and `legacy-cu118.constraints.txt` | Pinned OA/trlx/AlpacaFarm sources; trlx CUDA 11.8 environment; OA constraints | Install exact source commits editably/`--no-deps` so OA subpackages remain importable; preinstall pinned Torch/build tools for flash-attn; separately enumerate runtime requirements and constrain high-risk APIs pending a target-host lock | pip + network → explicit legacy source/runtime environment | None | Exact-ref text test, required imports, `pip check`, GPU smoke (target-host pending) | Install-time; source clones and large CUDA wheels | No runtime algorithm change; unresolved transitives remain documented |
